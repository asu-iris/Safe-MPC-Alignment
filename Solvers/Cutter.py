...
class cutter_v3(object):
    ...
    def construct_graph(self, horizon):
        ...
        B_grad = cd.gradient(B, traj_u_flat)
        self.B_grad_func = cd.Function('B_grad', [weights, init_state, traj_u_flat, target_r], [B_grad])

        J_grad = cd.gradient(J, traj_u_flat)
        self.J_grad_func = cd.Function('J_grad', [weights, init_state, traj_u_flat, target_r], [J_grad])

        marginal = -self.gamma * cd.log(-g_theta)
        m_grad = cd.gradient(marginal, traj_u_flat)
        self.m_grad_func = cd.Function('m_grad', [weights, init_state, traj_u_flat, target_r], [m_grad])

        phi_Jacobi = cd.jacobian(phi, traj_u_flat)
        self.phi_Jacobi_func = cd.Function('phi_jacobi', [init_state, traj_u_flat], [phi_Jacobi])

    def calc_planes(self, weights, init_state, traj_xu, human_corr, target_r):
        traj_u = []
        for i in range(self.horizon):
            start_idx = i * (self.x_dim + self.u_dim) + self.x_dim
            end_idx = start_idx + self.u_dim
        ...
        # print('M grad', self.m_grad_func(init_state,self.traj_u))
        # theta^T h <= b
        h = -np.dot(human_corr.T, self.J_grad_func(weights, init_state, traj_u, target_r)) * phi[1:]
        phi_jacobi = self.phi_Jacobi_func(init_state, traj_u)
        phi_jacobi_1_ = phi_jacobi[1:, :]
        h = h + self.gamma * (phi_jacobi_1_ @ human_corr)

        b = np.dot(human_corr.T, self.J_grad_func(weights, init_state, traj_u, target_r)) * phi[0]
        phi_grad_0 = phi_jacobi[0, :]
        b = b - self.gamma * (phi_grad_0 @ human_corr)
...
class cutter_v4(object):
    ...
    def construct_graph(self, horizon): # Construct the gradient functions in the expr of hyperplanes
        ...
        B_grad = cd.gradient(B, traj_u_flat)
        self.B_grad_func = cd.Function('B_grad', [weights, init_state, traj_u_flat, target_x], [B_grad])

        J_grad = cd.gradient(J, traj_u_flat)
        self.J_grad_func = cd.Function('J_grad', [weights, init_state, traj_u_flat, target_x], [J_grad])

        marginal = -self.gamma * cd.log(-g_theta)
        m_grad = cd.gradient(marginal, traj_u_flat)
        self.m_grad_func = cd.Function('m_grad', [weights, init_state, traj_u_flat, target_x], [m_grad])
    ...
    def calc_planes(self, weights, init_state, traj_xu, human_corr, target_x): # calculate hyperplanes
        ...
        # print('M grad', self.m_grad_func(init_state,self.traj_u))
        # theta^T h <= b
        h = -np.dot(human_corr.T, self.J_grad_func(weights, init_state, traj_u, target_x)) * phi[1:]
        phi_jacobi = self.phi_Jacobi_func(init_state, traj_u)
        phi_jacobi_1_ = phi_jacobi[1:, :]
        h = h + self.gamma * (phi_jacobi_1_ @ human_corr)

        b = np.dot(human_corr.T, self.J_grad_func(weights, init_state, traj_u, target_x)) * phi[0]
        phi_grad_0 = phi_jacobi[0, :]
        b = b - self.gamma * (phi_grad_0 @ human_corr)
