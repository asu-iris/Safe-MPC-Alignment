...
step_cost_vec=np.array([0.10,0.1,1,0.1,0.05])*1e-1
step_cost_f=uav_model.get_step_cost(step_cost_vec)
term_cost_vec=np.array([0.3,0.2,2,0.1])*1e-1
...
def generate_phi_x_1():
    ...
    phi=cd.vertcat(cd.DM(9),(x_pos_1-4)*(x_pos_1-4),(y_pos_1-4)*(y_pos_1-4))
...
controller.set_ctrl_param(4,[0,0,0,0],[3,3,3,3])
...
controller_2.set_ctrl_param(4,[0,0,0,0],[3,3,3,3])
controller_2.set_dyn(dyn_f)
controller_2.set_step_cost(step_cost_f)
controller_2.set_term_cost(term_cost_f)
#controller.construct_graph(horizon=Horizon)
#controller_2.set_g(phi_func_hard,weights=weights,gamma=0.1)
...
hb_calculator.set_g(phi_func,weights=weights,gamma=0.01)
