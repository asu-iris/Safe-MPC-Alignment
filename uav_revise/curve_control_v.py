...
from poly_feature import gen_poly_feats_single_v,gen_poly_feats_v
...
#####################################################################################
#constraint vector
phi_func = gen_poly_feats_v(Horizon=Horizon, con_idx=4, bias = -1)
...
while True:
    ...
    for i in range(500):
        ...
        if dist<=0.7:
            ...
            corr = -x_thrust * uav_v[0] - y_thrust * uav_v[1] - z_thrust*uav_v[2]
            print("direction",direction)
            corr_e = np.concatenate([corr.reshape(-1, 1), np.zeros((4 * (Horizon - 1), 1))])
            h, b, h_phi, b_phi = hb_calculator.calc_planes(weights, x, controller.opt_traj,
                                                                    human_corr=corr_e,
