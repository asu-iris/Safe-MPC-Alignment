...
def generate_rbf_quat_z(Horizon,x_center,x_half,ref_axis,num_q,z_min,z_max
                        ,num_z,bias=-2,epsilon_q=1,epsilon_z=1,z_factor=0.2,mode='default',softness=50):
    x_dim=7
    u_dim=6
    ...
    if mode=='default':
        ...
        for z in np.linspace(z_min,z_max,num_z):
            phi_list_p2.append(z_factor*sigmoid(x_half**2 - (next_x-x_center)**2,softness=softness)*rbf_general(epsilon=epsilon_z,dist=next_z-z))
    ...
    if mode=='cumulative':
        for z in np.linspace(z_min,z_max,num_z):
            ...
            for i in range(5):
                ...
                next_x = traj[(i+1)*(x_dim+u_dim)]
                phi_i += z_factor*sigmoid(x_half**2 - (next_x-x_center)**2,softness=softness)*rbf_general(epsilon=epsilon_z,dist=next_z-z) * disc
                disc *= 0.9
